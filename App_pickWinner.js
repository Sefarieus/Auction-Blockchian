...
class App extends Component {
  ...
  constructor(props) {
    ...
    window.ethereum.on('accountsChanged', async (account) => {
      ...
      // account is all lowcase. why?
      const accounts = await web3.eth.getAccounts();
      console.log("MetaMask account now is " + accounts[0]);
  ...
  // This might not be the best way to do, but it works!
  amIManager(address1, address2) {
  ...
  async pickWinner() {
    ...
    console.log("pick a winnder now....");
  ...
  render() {
    ...
    return (
      ...
      <div align="left">
          ...
          Total current pool is <b>{web3.utils.fromWei(this.state.balance, 'ether')} </b>ether.
        </p>
      </div>
      ...
      { this.amIManager(this.state.defaultAccount, this.state.manager)
        ?
          <div>
            <p>Since you are the contract mananger, you can do this</p>
      ...
      <div>
        <p><b>Current user is</b> {this.state.defaultAccount}</p>
      </div>
